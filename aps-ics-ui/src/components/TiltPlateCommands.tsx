/*
 * TiltPlate Commands section (SDD §4.3 "Command Section"). One row per command
 ...
 *
 * positionTiltPlate's x/y values are PUPIL-plane (M1) translations in mm; the
 * assembly converts them to stage moves via the per-axis stage->M1 factor.
 */
...
import {
  ...
  homeCmd,
  moveToDefaultCmd,
  positionTiltPlateCmd,
  stopCmd
} from '../models/tiltPlate'
import type { CmdName, PositionMethod, StatusSnapshot } from '../models/tiltPlate'
...
export const TiltPlateCommands = ({
  status,
  ready,
  busy,
  run
  ...
  return (
    <Space direction='vertical' size={6} style={{ width: '100%' }}>
      ...
      <Row
        label='Position tilt plate (pupil x, y)'
        enabled={on('positionTiltPlate')}
        onSubmit={() =>
          run(
            positionTiltPlateCmd(method, x, y),
            `positionTiltPlate [positioningMethod=${method}, xValue=${x}, yValue=${y}]`
          )
        }>
        ...
        />
        <InputNumber size='small' value={x} onChange={(v) => setX(Number(v ?? 0))} step={0.1} prefix='x' suffix='mm' style={{ width: 104 }} />
        <InputNumber size='small' value={y} onChange={(v) => setY(Number(v ?? 0))} step={0.1} prefix='y' suffix='mm' style={{ width: 104 }} />
      </Row>
