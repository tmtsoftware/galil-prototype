/*
 * PitFilterWheel Status section (SDD §4.3). Pure display of the assembly's PUBLISHED
...
import { PITFW_HCD_LABEL } from '../models/pitFilterWheel'
import type { AxisSnapshot, StatusSnapshot } from '../models/pitFilterWheel'
import type { SupervisorLifecycleState } from '@tmtsoftware/esw-ts'

const slotText = (n?: number): string =>
  n === undefined || n < 0 ? '—' : String(n)

export const PitFilterWheelStatus = ({
  status,
  axis,
  lifecycle
}: {
  ...
  <Space direction='vertical' size={10} style={{ width: '100%' }}>
    ...
    <MetaFooter
      items={[
        { label: 'HCD', value: <>{PITFW_HCD_LABEL} <Typography.Text type='secondary' style={{ fontSize: 11 }}>(config)</Typography.Text></> },
