/*
 * PSH Detector Commands section (SDD §5.3 "Command Section"). Commands grouped by
 * context (command kit): Cooling, Detector config (configure + set default),
 * Exposure and Recovery (recover + reset camera). Gating is unchanged —
 * `commandEnabled` mirrors the assembly validate gate; everything is disabled
 * while a command is in flight (`busy`). Submit + result logging live in Main.
      ...
      <CommandGroups>
        ...
        <CommandGroup title='Detector config'>
          <ParamCommand
            ...
            <Field label='Procedure ID'><Input size='small' value={procId} onChange={(e) => setProcId(e.target.value)} placeholder='procedureId' /></Field>
            <Field label='Observation ID'><Input size='small' value={obsId} onChange={(e) => setObsId(e.target.value)} placeholder='observationId' /></Field>
            {gainField()}
        ...
        <CommandGroup title='Exposure'>
          <ParamCommand
            name='Take exposure (→ shared disk)'
          ...
          <Actions>
            <ActionButton label='Store exposure (→ DMS)' enabled={on('storeExposure')} onSubmit={() => run(storeExposureCmd(), 'storeExposure')} />
