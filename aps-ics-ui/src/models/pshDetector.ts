/*
 * PSH Detector model (APS.ICS.PSH.Detector): constants, Setup builders, command
 * gating and the config snapshot. Names mirror PshDetectorKeys EXACTLY — PSH uses
 * `configureDetector` / `configureDetectorCooling`, `analogGainMode`, the
 * `rotStartCol` ROI typo, and configureDetector additionally carries
 * procedureId / observationId. There is NO takeAndStoreExposure.
 */
import { ComponentId, Prefix, Setup, booleanKey, choiceKey, floatKey, intKey, stringKey, Units } from '@tmtsoftware/esw-ts'
...
export const PSH_PREFIX_STR = 'APS.ICS.PSH.Detector'
export const PSH_PREFIX = Prefix.fromString(PSH_PREFIX_STR)
export const PSH_COMPONENT_ID = new ComponentId(PSH_PREFIX, 'Assembly')
...
export const PSH_CONFIG_PATH = 'APS/ICS/PSH/Detector.conf'
...
export const configureDetectorCoolingCmd = (setPoint: number): Setup =>
  new Setup(PSH_PREFIX, 'configureDetectorCooling', [floatKey('temperatureSetPoint', Units.degC).set([setPoint])])
...
export const configureDetectorCmd = (
  ...
  new Setup(PSH_PREFIX, 'configureDetector', [
    ...
    stringKey('procedureId').set([procedureId]),
    stringKey('observationId').set([observationId]),
...
export const setDefaultConfigurationCmd = (): Setup => new Setup(PSH_PREFIX, 'setDefaultConfiguration')

export const takeExposureCmd = (integrationTime: number, analogGainMode: AnalogGain): Setup =>
  new Setup(PSH_PREFIX, 'takeExposure', [
    floatKey('integrationTime', Units.second).set([integrationTime]),
    choiceKey('analogGainMode', ANALOG_GAINS).set(analogGainMode)
  ])

export const storeExposureCmd = (): Setup => new Setup(PSH_PREFIX, 'storeExposure')
export const abortExposureCmd = (): Setup => new Setup(PSH_PREFIX, 'abortExposure')

export const recoverCmd = (mode: RecoverMode, autoResume: boolean): Setup =>
  new Setup(PSH_PREFIX, 'recover', [
    choiceKey('mode', RECOVER_MODES).set(mode),
    booleanKey('autoResume').set([autoResume])
  ])

export const resetCameraCmd = (): Setup => new Setup(PSH_PREFIX, 'resetCamera')
...
export const PSH_DETECTOR_LABEL = 'Detector (Teledyne, MOCK)'

export const PSH_CONFIG_VIEW: ConfigSection[] = [
  {
    title: 'Detector — MOCK (SDD §5.3) — NOT calibrated',
    rows: [
      { label: 'Camera', value: 'Teledyne pupil/phasing (mock)' },
      { label: 'Image transfer', value: 'synthetic in-memory frame (no HCD)' },
      { label: 'takeExposure', value: 'FITS → APS Shared Disk + exposureStoreCompleted' },
      { label: 'storeExposure', value: 'archive → DMS + exposureArchiveCompleted' }
    ]
  },
  {
    title: 'Default ROI / binning (Table 5-14)',
    rows: [
  ...
  },
  {
    ...
    rows: [
      ...
      { label: 'CMS', value: 'OFF' },
      { label: 'Integration time', value: '1.0 s' },
