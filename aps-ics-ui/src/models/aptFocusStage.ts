/*
 * AptFocusStage model: constants, Setup builders, command gating and the config
 * snapshot for the APS.ICS.APT.FocusStage assembly.
 *
 ...
 * AptFocusStageKeys.scala EXACTLY.
 */
...
export const APTFS_PREFIX_STR = 'APS.ICS.APT.FocusStage'
export const APTFS_PREFIX = Prefix.fromString(APTFS_PREFIX_STR)
export const APTFS_COMPONENT_ID = new ComponentId(APTFS_PREFIX, 'Assembly')

// Event names published by AptFocusStageHandlers.publishTelemetry: status + a
// single axisStatus.
export const STATUS_EVENT = 'status'
export const AXIS_EVENT = 'axisStatus'

// Config Service path (prefix-mirrored: '.' -> '/', '<prefix>.conf').
export const APTFS_CONFIG_PATH = 'APS/ICS/APT/FocusStage.conf'
...
export const homeCmd = (): Setup => new Setup(APTFS_PREFIX, 'home')
export const configureCmd = (): Setup => new Setup(APTFS_PREFIX, 'configure')
export const moveToDefaultCmd = (): Setup => new Setup(APTFS_PREFIX, 'moveToDefaultPosition')
export const stopCmd = (): Setup => new Setup(APTFS_PREFIX, 'stop')
export const abortRecoveryCmd = (): Setup => new Setup(APTFS_PREFIX, 'abortErrorRecovery')
...
export const positionFocusStageCmd = (method: PositionMethod, mm: number): Setup =>
  new Setup(APTFS_PREFIX, 'positionFocusStage', [
...
// Bound HCD (container ComponentInfo connection): controller 3, channel D
// (SDD Fig 2-2). Static label.
export const APTFS_HCD_PREFIX_STR = 'APS.ICS.HCD.GalilMotion.3'
export const APTFS_HCD_LABEL = 'Galil HCD 3'

// Static mirror of ics-assemblies AptFocusStage.conf, shown when the Config
// Service has no seeded active version. Simulator values, NOT calibrated.
export const APTFS_CONFIG_VIEW: ConfigSection[] = [
  {
