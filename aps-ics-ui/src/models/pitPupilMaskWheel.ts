/*
 * PitPupilMaskWheel model: constants, Setup builders, command gating and the
 * config snapshot for the APS.ICS.PIT.PupilMaskWheel assembly (SDD §7 Wheel
 * Assemblies).
 *
 ...
 * Key names and choice domains mirror ics-assemblies PitPupilMaskWheelKeys.scala
...
export const PITPMW_PREFIX_STR = 'APS.ICS.PIT.PupilMaskWheel'
export const PITPMW_PREFIX = Prefix.fromString(PITPMW_PREFIX_STR)
export const PITPMW_COMPONENT_ID = new ComponentId(PITPMW_PREFIX, 'Assembly')

// Event names published by PitPupilMaskWheelHandlers.publishTelemetry: status + a
...
export const PITPMW_CONFIG_PATH = 'APS/ICS/PIT/PupilMaskWheel.conf'

// ---- Choice domains — must match ChoiceKey.make(...) in the assembly keys -----
export const PUPIL_MASKS = ['PH-1-1', 'Clear'] as const
...
export const homeCmd = (): Setup => new Setup(PITPMW_PREFIX, 'home')
export const configureCmd = (): Setup => new Setup(PITPMW_PREFIX, 'configure')
export const moveToDefaultCmd = (): Setup =>
  new Setup(PITPMW_PREFIX, 'moveToDefaultPosition')
export const stopCmd = (): Setup => new Setup(PITPMW_PREFIX, 'stop')
export const abortRecoveryCmd = (): Setup =>
  new Setup(PITPMW_PREFIX, 'abortErrorRecovery')
...
export const selectPupilMaskCmd = (pupilMask: PupilMask): Setup =>
  new Setup(PITPMW_PREFIX, 'selectPupilMask', [
...
export const positionWheelCmd = (positionNumber: WheelPosition): Setup =>
  new Setup(PITPMW_PREFIX, 'positionWheel', [
...
export const positionMotorCmd = (
  ...
  new Setup(PITPMW_PREFIX, 'positionMotor', [
...
export const commandDetentCmd = (position: DetentPosition): Setup =>
  new Setup(PITPMW_PREFIX, 'commandDetent', [
...
// Bound HCD (container ComponentInfo connection): controller 1, channel E
// (SDD Fig 2-2). Static label.
export const PITPMW_HCD_PREFIX_STR = 'APS.ICS.HCD.GalilMotion.1'
export const PITPMW_HCD_LABEL = 'Galil HCD 1'

// Static mirror of ics-assemblies PitPupilMaskWheel.conf, shown when the Config
...
export const PITPMW_CONFIG_VIEW: ConfigSection[] = [
  {
  ...
  },
  {
    ...
    rows: [
      { label: 'Position 1', value: 'PH-1-1' },
      { label: 'Position 2', value: 'Clear' }
    ]
  },
  {
    title: 'Detent DIO (SDD §7) — DUMMY placeholders, engineering-only',
    rows: [
      { label: 'Output bit (solenoid)', value: '3 (Galil 1-based)' },
      { label: 'Extended sensor input', value: '3 (Galil 1-based)' },
      { label: 'Retracted sensor input', value: '4 (Galil 1-based)' }
    ]
  }
]
