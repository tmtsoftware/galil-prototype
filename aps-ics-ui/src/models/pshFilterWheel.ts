/*
 * PshFilterWheel model: constants, Setup builders, command gating and the config
 * snapshot for the APS.ICS.PSH.FilterWheel assembly (SDD §7.4).
 *
 ...
 * (low-level engineering move); the PSH-specific command is selectFilter.
 ...
 * PshFilterWheelKeys.scala EXACTLY — a mismatched name or key TYPE yields
...
export const PSHFW_PREFIX_STR = 'APS.ICS.PSH.FilterWheel'
export const PSHFW_PREFIX = Prefix.fromString(PSHFW_PREFIX_STR)
export const PSHFW_COMPONENT_ID = new ComponentId(PSHFW_PREFIX, 'Assembly')

// Event names published by PshFilterWheelHandlers.publishTelemetry: status + a
// single axisStatus.
export const STATUS_EVENT = 'status'
export const AXIS_EVENT = 'axisStatus'

// Config Service path (prefix-mirrored: '.' -> '/', '<prefix>.conf').
export const PSHFW_CONFIG_PATH = 'APS/ICS/PSH/FilterWheel.conf'
...
export const homeCmd = (): Setup => new Setup(PSHFW_PREFIX, 'home')
export const configureCmd = (): Setup => new Setup(PSHFW_PREFIX, 'configure')
export const moveToDefaultCmd = (): Setup =>
  new Setup(PSHFW_PREFIX, 'moveToDefaultPosition')
export const stopCmd = (): Setup => new Setup(PSHFW_PREFIX, 'stop')
export const abortRecoveryCmd = (): Setup =>
  new Setup(PSHFW_PREFIX, 'abortErrorRecovery')

// selectFilter(filter): resolve a filter name to its assigned wheel slot.
export const selectFilterCmd = (filter: Filter): Setup =>
  new Setup(PSHFW_PREFIX, 'selectFilter', [
...
export const positionWheelCmd = (positionNumber: WheelPosition): Setup =>
  new Setup(PSHFW_PREFIX, 'positionWheel', [
...
export const positionMotorCmd = (
  ...
  new Setup(PSHFW_PREFIX, 'positionMotor', [
...
// Bound HCD (container ComponentInfo connection): controller 1, channel B
// (SDD Fig 2-2). Static label.
export const PSHFW_HCD_PREFIX_STR = 'APS.ICS.HCD.GalilMotion.1'
export const PSHFW_HCD_LABEL = 'Galil HCD 1'

// Static mirror of ics-assemblies PshFilterWheel.conf, shown when the Config
...
export const PSHFW_CONFIG_VIEW: ConfigSection[] = [
  {
