...
/**
 * Common base for APS-ICS stage (and, later, wheel/K-Mirror) assemblies.
 * Implements the shared CSW wiring and the SDD §6.1 behaviour:
 ...
 *
 * Subclasses provide the config resource, the list of axis config keys, the
 * assembly-specific commands, and telemetry publication (which uses the
 * assembly's own generated event keys).
 *
 ...
 * apsAlertEvent rather than running the retry/home ladder. abortErrorRecovery is
 * a no-op stub. The full recovery machine is a follow-on once the happy path is
 * proven on hardware.
 */
abstract class StageAssemblyHandlers(ctx: ActorContext[TopLevelActorMessage], cswCtx: CswContext)
  ...
  private def onHcdAxisState(curr: CurrentState): Unit =
    ...
    axes.find(_.galilChannel == channel).foreach { a =>
      if sn.startsWith("CurrentStateAxis") then
        ...
        latestAxis = latestAxis + (a.name -> snap)
        reconcileOperationalState()
  ...
  /** configure: configAxis + configLinearAxis/configRotatingAxis per axis.
   *  All mm-based config values are converted to controller counts here. */
  protected def runConfigure(): Future[SubmitResponse] =
    submitAllAxes { a =>
      val cfg = hcdSetup(
        ...
        Hcd.ConfigAxisCommand.velocityKey.set((a.velocity * a.countsPerMm).toFloat),
        Hcd.ConfigAxisCommand.accelerationKey.set((a.acceleration * a.countsPerMm).toFloat),
        Hcd.ConfigAxisCommand.decelerationKey.set((a.deceleration * a.countsPerMm).toFloat),
        Hcd.ConfigAxisCommand.indexOffsetKey.set(a.mmToCounts(a.indexOffsetMm).toFloat),
        Hcd.ConfigAxisCommand.indexSpeedKey.set((a.indexSpeed * a.countsPerMm).toFloat),
        Hcd.ConfigAxisCommand.inPositionThresholdKey.set(a.mmToCounts(a.inPositionThresholdMm).toFloat)
      )
      val shape =
        if a.isRotational then
          ...
          hcdSetup(
            ...
            Hcd.ConfigLinearAxisCommand.lowerLimitKey.set(a.mmToCounts(a.lowerLimitMm).toFloat),
            Hcd.ConfigLinearAxisCommand.upperLimitKey.set(a.mmToCounts(a.upperLimitMm).toFloat)
          )
