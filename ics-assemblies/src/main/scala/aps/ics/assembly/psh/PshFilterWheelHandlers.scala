...
import aps.ics.assembly.icd.PshFilterWheelKeys.`ICS.PSH.FilterWheel` as FW

import scala.jdk.CollectionConverters._

/**
 * APS PSH Filter Wheel Assembly (APS.ICS.PSH.FilterWheel), SDD §7.4.
 *
 * A single ROTATING axis ("filterWheel") on Galil controller 1, channel B. The
 * common wheel base (WheelAssemblyHandlers) supplies configure / home /
 ...
 * the PSH-specific selectFilter command, which resolves a filter NAME to a wheel
 ...
 * here yet — that is the InputOutputState subscription, deferred with the
 * pupil-mask detent work.
 */
class PshFilterWheelHandlers(ctx: ActorContext[TopLevelActorMessage], cswCtx: CswContext)
    extends WheelAssemblyHandlers(ctx, cswCtx):

  import cswCtx._

  override protected def configResource: String      = "PshFilterWheel.conf"
...
object PshFilterWheelApp:
  def main(args: Array[String]): Unit =
    val defaultConfig = ConfigFactory.load("PshFilterWheelContainer.conf")
    ContainerCmd.start("PshFilterWheel", Subsystem.APS, args, Some(defaultConfig))