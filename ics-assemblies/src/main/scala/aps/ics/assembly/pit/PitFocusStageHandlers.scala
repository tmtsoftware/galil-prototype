...
import aps.ics.assembly.icd.PitFocusStageKeys.`ICS.PIT.FocusStage` as FS

import scala.concurrent.Future

/**
 * APS PIT Focus Stage Assembly (APS.ICS.PIT.FocusStage), SDD §6.6.
 *
 ...
 * error-recovery and telemetry plumbing. There is no PIT-specific configuration
 * (SDD §6.6.2.2: None).
 *
 * Per SDD Figure 2-2 the stage motor is on Galil controller 1, channel D.
 */
class PitFocusStageHandlers(ctx: ActorContext[TopLevelActorMessage], cswCtx: CswContext)
    extends StageAssemblyHandlers(ctx, cswCtx):

  import cswCtx._

  override protected def configResource: String      = "PitFocusStage.conf"
...
object PitFocusStageApp:
  def main(args: Array[String]): Unit =
    val defaultConfig = ConfigFactory.load("PitFocusStageContainer.conf")
    ContainerCmd.start("PitFocusStage", Subsystem.APS, args, Some(defaultConfig))
