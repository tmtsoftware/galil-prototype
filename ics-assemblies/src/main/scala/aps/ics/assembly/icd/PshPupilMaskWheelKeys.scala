...
object PshPupilMaskWheelKeys {
  val subsystem = "APS"

  /** API for Assembly: APS.ICS.PSH.PupilMaskWheel */
  object `ICS.PSH.PupilMaskWheel` {
    val prefix: Prefix = Prefix(Subsystem.APS, "ICS.PSH.PupilMaskWheel")

    /** PSH PupilMaskWheel status event */
    object StatusEvent {
    ...
    /** PSH PupilMaskWheel axis status event */
    object AxisStatusEvent {
      ...
      /** The current detent state */
      val detentStateKey: GChoiceKey = ChoiceKey.make("detentState", "RETRACTED", "EXTENDED", "OUT OF POSITION")

    }

    /**
     * PSH PupilMaskWheel axis config event, published only when configAxis or configRotatingAxis cmd has completed during
     * assembly startup
     */
    ...
    /** Selects a pupil mask to position into the light beam. */
    object SelectPupilMaskCommand {
      val commandName: CommandName = CommandName("selectPupilMask")

      /** The name of the pupilMask to position into the beam */
      val pupilMaskKey: GChoiceKey = ChoiceKey.make("pupilMask", "PH-2-0", "SH-0", "SH-2", "SH-5", "Clear")
    ...
    /** Positions the wheel detent. Used for diagnostic and debugging purposes. */
    object CommandDetentCommand {
      val commandName: CommandName = CommandName("commandDetent")

      /** The desired detent position */
      val positionKey: GChoiceKey = ChoiceKey.make("position", "EXTENDED", "RETRACTED")
