...
object AptFilterWheelKeys {
  val subsystem = "APS"

  /** API for Assembly: APS.ICS.APT.FilterWheel */
  object `ICS.APT.FilterWheel` {
    val prefix: Prefix = Prefix(Subsystem.APS, "ICS.APT.FilterWheel")

    /** APT FilterWheel status event */
    object StatusEvent {
    ...
    /** APT FilterWheel axis status event */
    ...
    /**
     * APT FilterWheel axis config event, published only when configAxis or configRotatingAxis cmd has completed during assembly
     * startup
     */
    ...
    /** Positions the wheel in degrees or motor in steps. Used for command calibration, diagnostic and debugging purposes. */
    ...
    object SelectFilterCommand {
      ...
      val filterKey: GChoiceKey = ChoiceKey.make("filter", "ND1", "ND2", "NB589", "OPEN")
    ...
    object PositionWheelCommand {
      ...
      val positionNumberKey: GChoiceKey = ChoiceKey.make("positionNumber", "1", "2", "3", "4")
