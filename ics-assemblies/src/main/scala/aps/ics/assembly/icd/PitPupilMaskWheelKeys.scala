...
object PitPupilMaskWheelKeys {
  val subsystem = "APS"

  /** API for Assembly: APS.ICS.PIT.PupilMaskWheel */
  object `ICS.PIT.PupilMaskWheel` {
    val prefix: Prefix = Prefix(Subsystem.APS, "ICS.PIT.PupilMaskWheel")

    /** PIT PupilMaskWheel status event */
    object StatusEvent {
    ...
    /** PIT PupilMaskWheel axis status event */
    ...
    /**
     * PIT PupilMaskWheel axis config event, published only when configAxis or configRotatingAxis cmd has completed during
     * assembly startup
     */
    ...
    object SelectPupilMaskCommand {
      ...
      /** The name of the pupil mask to position into the beam */
      val pupilMaskKey: GChoiceKey = ChoiceKey.make("pupilMask", "PH-1-1", "Clear")
