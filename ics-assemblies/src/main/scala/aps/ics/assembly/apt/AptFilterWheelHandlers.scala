...
import aps.ics.assembly.icd.AptFilterWheelKeys.`ICS.APT.FilterWheel` as FW

import scala.jdk.CollectionConverters._

/**
 * APS APT Filter Wheel Assembly (APS.ICS.APT.FilterWheel), SDD §7 (Wheel Assemblies).
 *
 * A single ROTATING axis ("filterWheel") on Galil controller 3, channel E (SDD
 * Figure 2-2; HCD GalilHcdConfig-APS-3.conf axis E, "aptFilterWheel"). The common
 ...
 * the APT-specific selectFilter command, which resolves a filter NAME to a wheel
 * slot via the Wheel Position N Assignment config (SDD Table 7-1).
 *
 * Structurally identical to PshFilterWheelHandlers; the APT wheel has FOUR slots
 * (ND1, ND2, NB589, OPEN) rather than seven. The base's local positionNumber key
 * spans 1..7 and reads APT's 1..4 demands fine (the choice set is metadata; only
 * name+type are matched), so no override is needed for the narrower slot range.
 *
 ...
 * detent work.
 */
class AptFilterWheelHandlers(ctx: ActorContext[TopLevelActorMessage], cswCtx: CswContext)
    extends WheelAssemblyHandlers(ctx, cswCtx):

  import cswCtx._

  override protected def configResource: String      = "AptFilterWheel.conf"
...
object AptFilterWheelApp:
  def main(args: Array[String]): Unit =
    val defaultConfig = ConfigFactory.load("AptFilterWheelContainer.conf")
    ContainerCmd.start("AptFilterWheel", Subsystem.APS, args, Some(defaultConfig))
