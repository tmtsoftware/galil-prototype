...
import aps.ics.assembly.icd.AptFocusStageKeys.`ICS.APT.FocusStage` as FS

import scala.concurrent.Future

/**
 * APS APT Focus Stage Assembly (APS.ICS.APT.FocusStage), SDD §6.6.
 *
 ...
 * error-recovery and telemetry plumbing. There is no APT-specific configuration
 * (SDD §6.6.2.2: None).
 *
 * Per SDD Figure 2-2 the stage motor is on Galil controller 3, channel D.
 */
class AptFocusStageHandlers(ctx: ActorContext[TopLevelActorMessage], cswCtx: CswContext)
    extends StageAssemblyHandlers(ctx, cswCtx):

  import cswCtx._

  override protected def configResource: String      = "AptFocusStage.conf"
...
object AptFocusStageApp:
  def main(args: Array[String]): Unit =
    val defaultConfig = ConfigFactory.load("AptFocusStageContainer.conf")
    ContainerCmd.start("AptFocusStage", Subsystem.APS, args, Some(defaultConfig))
